...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem }) => {
    ...
    const handleLike = async () => {
        if (!user) {
            toast.error('Please login to like a problem', options);
            return;
        }
        ...
        // 3 Cases: 1. Already Liked 2. Already Disliked 3. Neutral
        await runTransaction(firestore, async (transaction) => {
            const { userRef, userDoc, problemRef, problemDoc } = await returnUserAndProblemData(transaction);

            if (userDoc.exists() && problemDoc.exists()) {
                if (liked) {
                    // Already liked, so click should remove like and decrement like count
                    ...
                    // Already disliked, so click should remove dislike, add like and increment like count
                    transaction.update(userRef, {
                    ...
                    // Neutral, so click should add like and increment like count
                    transaction.update(userRef, {
                        likedProblems: [...userDoc.data().likedProblems, problem.id]
                    });
                    transaction.update(problemRef, {
    ...
    const handleDislike = async () => {
        if (!user) {
            toast.error('Please login to dislike a problem', options);
            return;
        }
        ...
        // 3 Cases: 1. Already Disliked 2. Already Liked 3. Neutral
        await runTransaction(firestore, async (transaction) => {
            const { userRef, userDoc, problemRef, problemDoc } = await returnUserAndProblemData(transaction);
    ...
    const handleStar = async () => {
        if (!user) {
            toast.error('Please login to star a problem', options);
            return;
        }
        ...
        // 2 Cases: 1. Already Starred 2. Neutral
        if (!starred) {
            const userRef = doc(firestore, 'users', user.uid);
